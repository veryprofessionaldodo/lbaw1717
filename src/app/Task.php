...
class Task extends Model
{
  ...
  public function isUserAssigned($user_id){
    return DB::select(
      DB::raw('SELECT task_state_record.user_completed_id FROM task_state_record
      WHERE task_state_record.task_id = :task_id
      AND task_state_record.state = :state1
      AND task_state_record.user_completed_id = :user_id
      AND NOT EXISTS (SELECT * FROM task_state_record a WHERE a.task_id = task_state_record.task_id 
      AND a.user_completed_id = task_state_record.user_completed_id
      AND a.state = :state2 AND a.date > task_state_record.date)'),
      array('task_id' => $this->id, 'state1' => 'Assigned', 
      'state2' => 'Unassigned', 'user_id' => $user_id));
  }
